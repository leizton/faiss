...
struct IndexIVFFastScan : IndexIVF {
    ...
    struct CoarseQuantized {
        ...
        CoarseQuantized() : nprobe(0u), dis(nullptr), ids(nullptr) {}

        CoarseQuantized(size_t a) : nprobe(a), dis(nullptr), ids(nullptr) {}

        CoarseQuantized(size_t a, const float* b, const idx_t* c) : nprobe(a), dis(b), ids(c) {}
    };
